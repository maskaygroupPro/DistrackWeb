...
$leyenda = array("Entregados","Otros");
		$qu="select distinct date(b.fechaprog) from ddaryza.pedidos b where b.fechaprog<=curdate() order by date(b.fechaprog) desc limit 7;";
		$re=mysql_query($qu) or die("Sesion Expirada 1");
        	$p1='';
            $p2='';	
            $p3='';	
            $p4='';	
		...
		while($fi=mysql_fetch_array($re))
			{
				...
				$noe=0;
				$fec=$fi[0];
		//$que1="select count(b.estado) from ddaryza.pedidos b,gpslog.livedata l where b.fechaprog<=curdate() and b.placa=l.vehicleReg and b.placa in(select l.VehicleReg from gpslog.netusers n,gpslog.system_users s,gpslog.livedata l where l.VehicleID=n.UIN and n.NetUserUIN=s.ID and b.estado='Entregado OTIF' and s.ID=".$_SESSION['ID'].") and b.fechaprog='".$fec."'";			
		$que1="select count(b.estado) from ddaryza.pedidos b where b.estado='Entregado OTIF' and date(b.fechaprog)='".$fec."'";
		$res=mysql_query($que1) or die("Sesion Expirada2");
		$ro=mysql_fetch_array($res);
		$ent=$ro[0];
		//$que0="select count(b.estado) from ddaryza.pedidos b,gpslog.livedata l where b.fechaprog<=curdate() and b.placa=l.vehicleReg and b.placa in(select l.VehicleReg from gpslog.netusers n,gpslog.system_users s,gpslog.livedata l where l.VehicleID=n.UIN and n.NetUserUIN=s.ID and b.estado<>'Entregado OTIF' and s.ID=".$_SESSION['ID'].") and b.fechaprog='".$fec."'";
		$que0="select count(b.estado) from ddaryza.pedidos b where b.estado<>'Entregado OTIF' and date(b.fechaprog)='".$fec."'";
		$res1=mysql_query($que0) or die("Sesion Expirada3");
