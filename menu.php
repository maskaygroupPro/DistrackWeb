...
<body>
<?php
$cmps="b.numpedido,b.idpedido,b.documento,left(b.cliente,30),b.volumen,left(b.distcliente,15),b.estado,b.fecentrega,b.horentrega,left(b.localpedido,25),b.placa,b.latitud,b.longitud,b.ventanaini,b.ventanafin,b.fechaprog,b.peso,b.aux1,b.orden,b.aux3,b.aux1,left(b.dircliente,30),left(b.refcliente,25),b.observacion,b.fot_foto,b.motivo ";
$tabl="ddaryza.pedidos b";
$cond="b.fechaprog=current_date";
$query = "select ".$cmps." from ".$tabl." where ".$cond." order by b.placa,b.orden ; ";
...
if ($NO > 0){
	for ($i=1;$i<=$NO;$i++){
		if ($i == 1){
			...
			echo "<td nowrap class='RecSrcGridHeaderLLSort' align='center'>Motivo</td>";
			...
			echo "<td nowrap class='RecSrcGridHeaderLLSort' align='center'>GPS</td>";
			echo "<td nowrap class='RecSrcGridHeaderLLSort' align='center'>Foto</td>";
		...
		echo "<td nowrap class='GridAttrField' align='left'>".$DOCUMENTO[$i]."</td>";
		...
		echo "<td nowrap class='GridAttrField' align='left'>".$MOTIVO[$i]."</td>";
		...
		if (	$READX[$i]	<> '' && $READY[$i]	<> '' ){
			...
			$Ubic = "show_map.php?var1=".HTML_filtro($READX[$i])."&var2=".HTML_filtro($READY[$i])."&var3=".HTML_filtro($DOCUMENTO[$i])." / ".HTML_filtro($PEDIDO[$i]);
		...
		if (!is_null($FFOTO[$i])){
			echo "<td nowrap class='GridAttrField'  align='center'>";
			$Ubic = "show_galeria_app.php?var0=".HTML_filtro($DOCUMENTO[$i]);
			echo " <a HREF=\""."javascript:void(window.open('".$Ubic."&dummy=.pdf','_blank', 'scrollbars=yes,toolbar=no,resizable=yes,menubar=no,location=no'))\""."><img src='images/foto.png' height='10' width='10'></a>";
			echo "</td>";
		}else{
			echo "<td nowrap class='GridAttrField' align='left'></td>";
		}
